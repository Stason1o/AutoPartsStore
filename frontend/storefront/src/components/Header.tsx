...
function LocaleSwitcher() {
  ...
  const switchTo = (next: Locale) => {
    ...
    router.push(`/${next}${rest}${typeof window !== 'undefined' ? window.location.search : ''}`);
  };

  return (
    <span style={{ display: 'inline-flex', gap: 2, fontFamily: T.mono, fontSize: 12.5, fontWeight: 700 }}>
      {LOCALES.map((l, i) => (
        <span key={l} style={{ display: 'inline-flex', alignItems: 'center' }}>
          {i > 0 && <span style={{ color: '#3c424b', margin: '0 6px' }}>|</span>}
          <button onClick={() => switchTo(l)} style={{ background: 'none', border: 0, cursor: l === locale ? 'default' : 'pointer', color: l === locale ? '#fff' : '#6b7480', fontFamily: 'inherit', fontWeight: 700, fontSize: 12.5, padding: 0, letterSpacing: '.06em' }}>
            {l.toUpperCase()}
          </button>
        </span>
      ))}
    ...
    <>
      ...
      <div style={{ background: T.g900, color: '#aab1bd', fontSize: 12.5, borderBottom: `1px solid ${T.g750}` }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '7px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 22, whiteSpace: 'nowrap', flexShrink: 0 }}>
            <LocaleSwitcher />
            <span style={{ color: '#6b7480' }}>{t.hours}</span>
      ...
      <header style={{ background: T.g800, color: '#fff', position: 'sticky', top: 0, zIndex: 40, borderBottom: `1px solid ${T.g700}`, boxShadow: '0 1px 0 rgba(0,0,0,.4)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 28 }}>
          <Link href={lp('/')} style={{ textDecoration: 'none' }}><Logo /></Link>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
            <Link href={lp('/cart')} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 10, background: T.accent, color: '#fff', border: 0, borderRadius: 9, height: 46, padding: '0 18px 0 16px', fontWeight: 700, fontSize: 14, cursor: 'pointer', textDecoration: 'none' }}>
        ...
        {car && (
          <div style={{ background: T.g850, borderTop: `1px solid ${T.g700}` }}>
            <div style={{ maxWidth: 1280, margin: '0 auto', padding: '10px 24px', display: 'flex', alignItems: 'center', gap: 14 }}>
              <span style={{ fontFamily: T.mono, fontSize: 11, letterSpacing: '.16em', color: T.muted2, textTransform: 'uppercase' }}>{t.yourCar}</span>
              <Link href={lp(`/catalog?vehicleId=${car.vehicleId}`)} style={{ display: 'flex', alignItems: 'center', gap: 10, background: T.g700, border: `1px solid ${T.lineD}`, borderRadius: 8, padding: '7px 14px', textDecoration: 'none' }}>
