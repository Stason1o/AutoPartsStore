...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!p) return { title: dict.meta.productNotFound };
  const description = `${p.name}${p.brand ? ` (${p.brand})` : ''}. ${p.available > 0 ? dict.meta.inStock : dict.meta.checkStock} ${dict.meta.price}: ${p.price ?? '—'} MDL. ${dict.meta.sku} ${p.sku}.`;
  return {
    title: `${p.name} — ${dict.meta.sku} ${p.sku}`,
    description,
    alternates: {
      canonical: `/${locale}/product/${p.slug}`,
      languages: { ro: `/ro/product/${p.slug}`, ru: `/ru/product/${p.slug}` },
    },
...
export default async function ProductPage({ params }: Props) {
  ...
  const tc = getDict(locale).catalog;
  const p = await loadProduct(slug);
  if (!p) notFound();
  ...
  return (
    <div className="sc-in" style={{ padding: '24px 0 56px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: T.muted, marginBottom: 18, fontFamily: T.mono, flexWrap: 'wrap' }}>
        <Link href={`/${locale}`} style={{ color: T.muted, textDecoration: 'none' }}>{tc.home}</Link>
        <span>/</span>
        <Link href={p.categoryId ? `/${locale}/catalog?categoryId=${p.categoryId}` : `/${locale}/catalog`} style={{ color: T.muted, textDecoration: 'none' }}>
          {p.categoryName ?? tc.allParts}
        </Link>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 36 }}>
        <div style={{ background: T.paper, border: `1px solid ${T.line}`, borderRadius: 14, padding: 24 }}>
          <h3 style={{ fontSize: 17, fontWeight: 700, margin: '0 0 12px' }}>{t.description}</h3>
          <p style={{ fontSize: 14.5, color: '#3c424b', lineHeight: 1.65, margin: 0 }}>
            {p.description || t.descriptionFallback}
          </p>
        </div>
        <div style={{ background: T.paper, border: `1px solid ${T.line}`, borderRadius: 14, padding: 24 }}>
          <h3 style={{ fontSize: 17, fontWeight: 700, margin: '0 0 14px' }}>{t.applicability}</h3>
          {p.fitsVehicles.length === 0 ? (
            <p style={{ fontSize: 14, color: T.muted, margin: 0 }}>{t.applicabilityFallback}</p>
          ) : (
            <div>
