...
export default function CartPage() {
  ...
  return (
    <div className="sc-in" style={{ padding: '24px 0 56px' }}>
      <h1 style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-.01em', margin: '0 0 24px' }}>{t.title}</h1>
      ...
      {cart.length === 0 ? (
        <div style={{ background: T.paper, border: `1px solid ${T.line}`, borderRadius: 16, padding: '64px 32px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 19, fontWeight: 700, marginBottom: 8 }}>{t.empty}</div>
          <div style={{ fontSize: 14, color: T.muted, marginBottom: 24 }}>{t.emptyHint}</div>
          <Link href={lp('/catalog')} style={{ display: 'inline-block', background: T.accent, color: '#fff', borderRadius: 10, padding: '13px 26px', fontWeight: 700, fontSize: 14.5, textDecoration: 'none' }}>
            {t.goCatalog}
          </Link>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {cart.map(it => (
              <div key={it.productId} style={{ background: T.paper, border: `1px solid ${T.line}`, borderRadius: 14, padding: 16, display: 'flex', gap: 16, alignItems: 'center' }}>
                <Link href={lp(`/product/${it.slug}`)} className="placeholder-stripes" style={{ width: 90, height: 90, borderRadius: 10, border: `1px solid ${T.line}`, flexShrink: 0, display: 'grid', placeItems: 'center', textDecoration: 'none' }}>
                  <span style={{ fontFamily: T.mono, fontSize: 9, letterSpacing: '.08em', color: T.muted2 }}>FOTO</span>
                </Link>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <Link href={lp(`/product/${it.slug}`)} style={{ textDecoration: 'none', fontSize: 15, fontWeight: 600, color: T.ink, lineHeight: 1.35 }}>{it.name}</Link>
                  <div style={{ fontFamily: T.mono, fontSize: 12, color: T.muted, marginTop: 4 }}>{dict.product.sku} {it.sku}{it.brand ? ` · ${it.brand}` : ''}</div>
                  <div style={{ fontFamily: T.mono, fontSize: 13, color: T.muted, marginTop: 6 }}>{fmt(it.price)} MDL {t.perPiece}</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12, flexShrink: 0 }}>
                  ...
                  <button onClick={() => removeItem(it.productId)} style={{ background: 'none', border: 0, color: T.muted, fontSize: 12.5, cursor: 'pointer', padding: 0 }}>{t.remove}</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <aside style={{ position: 'sticky', top: 96, background: T.paper, border: `1px solid ${T.line}`, borderRadius: 16, padding: 22 }}>
            <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 16 }}>{t.total}</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: T.muted, marginBottom: 16, gap: 10 }}>
              <span>{t.delivery}</span><span style={{ fontSize: 13, textAlign: 'right' }}>{t.deliveryAtCheckout}</span>
            </div>
            <div style={{ borderTop: `1px solid ${T.line}`, paddingTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 18, gap: 10 }}>
              <span style={{ fontWeight: 700, fontSize: 16 }}>{t.toPay}</span>
            ...
            <Link href={lp('/checkout')} style={{ display: 'block', textAlign: 'center', lineHeight: '52px', height: 52, background: T.accent, color: '#fff', borderRadius: 11, fontWeight: 700, fontSize: 15.5, textDecoration: 'none' }}>
              {t.checkout}
            </Link>
            <Link href={lp('/catalog')} style={{ display: 'block', textAlign: 'center', marginTop: 10, color: T.muted, fontSize: 13.5, textDecoration: 'none', padding: 8 }}>
              {t.continueShopping}
            </Link>
          </aside>
        </div>
